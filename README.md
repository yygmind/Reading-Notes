...
1.  一个div(200px*200px)在左侧，另一个div自适应在右侧

2. 一个未知宽高元素怎么上下左右垂直居中
...
6. session、cookie、sessionStorage、localStorage等区别
...
10. css和HTML 问的的确都不是很多，然后还有什么标签，meta和media啥的，大概也就介绍了下，问的都不是很深，我也没有回答的很深。。。因为我HTML CSS真的一般般。
...
12.  1.css 盒模型
      2.css布局，左边定宽右边自适应。两种方法，NEC上的用负边距消除宽度，用弹性布局。然后问我有没有第三种。。。
...
13.   rem是什么？em是什么？如果上一层就是根root了，em和rem等价么？
...
2. <a name='js_4'>写一个js的通用事件绑定函数</a>
3. 原型链，对象，构造函数之间的一些联系
4. DOM事件的绑定的几种方式
...
2. ES6的箭头函数this问题，以及拓展运算符。

        这一块主要是API和概念的问题，扯了一些规范以及严格模式下其他情况this只想问题。

3. JS模块化Commonjs,UMD,CMD规范的了解，以及ES6的模块化跟其他几种的区别，以及出现的意义。

        这个也是说了一下自己的理解和认知，自己对模块化历史以及一些规范都有所涉猎，这一块也还凑合。        

4. 白板写代码，用最简洁的代码实现数组去重。
...
2. 原型链的问题，内部原理，继承的方式之类

3. =＝和===的区别？＝＝怎么进行类型转换的，说说有哪几种情况？
...
16. 实现一个函数，验证一个字符串，长度8位，至少包括其中三种或四种（数字，大写，小写，_）

17. A->B->C,A看向B,B看向C，已知A已婚,C未婚，是否一定存在这样的关系：已婚直接看向未婚 （1）存在（2）不存在（3）不确定

18.  3.冒泡和捕获，事件流哪三个阶段？除了冒泡和捕获，还有目标阶段。他们的先后顺序，先捕获，到了目标，再冒泡。（不要只记概念，要了解是干么用的）
...
4. 对象中key-value的value怎么再放一个对象。（直接放也可以，转成json字符串存数，读取再解析）
